...
def save_triangle_mesh(fn, vertices, faces, vertex_colors=None):
    # if assets are batched, select the first one
    if len(vertices.shape) == 3:
        vertices = vertices[0]
    if len(faces.shape) == 3:
        faces = faces[0]
    if vertex_colors is not None and len(vertex_colors) == 3:
    ...
    o3d.io.write_triangle_mesh(fn, m)

def save_point_cloud(fn, points, colors=None):
    # if assets are batched, select the first one
    ...
    m.points = o3d.utility.Vector3dVector(points)
    if colors is not None:
        m.colors = o3d.utility.Vector3dVector(colors)
    o3d.io.write_point_cloud(fn, m)

def save_tetrahedron(fn, vertices, all_edges):
    if isinstance(vertices, torch.Tensor):
        vertices = vertices.detach().cpu().numpy()
    if isinstance(all_edges, torch.Tensor):
        all_edges = all_edges.detach().cpu().numpy()

    m = o3d.geometry.LineSet()
    m.points = o3d.utility.Vector3dVector(vertices)
    m.lines = o3d.utility.Vector2iVector(all_edges)
    o3d.io.write_line_set(fn, m)
